...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        alp=motion[0]
        dis=motion[1]
        steer2=random.gauss(alp,self.steering_noise)
        dist2=random.gauss(dis,self.distance_noise)
        B=(dist2/self.length)*tan(steer2)
        # print self.x,self.y,self.orientation
        if B>0.001:
            R=dist2/B
            x=self.x-sin(self.orientation)*R+sin(self.orientation+B)*R
            y=self.y+cos(self.orientation)*R-cos(self.orientation+B)*R
            orientation=(self.orientation+B)%(2*pi)
            
        else:
            x=self.x+dist2*cos(self.orientation)
            y=self.y+dist2*sin(self.orientation)
            orientation=(self.orientation+B)%(2*pi)
    ...
    def sense(self,add_noise): #do not change the name of this function
        ...
        for pos in landmarks:
            del_x=pos[1]-self.x
            del_y=pos[0]-self.y
            ang=atan2(del_y,del_x)-self.orientation
            if add_noise:
                ang+=random.gauss(0.0,self.bearing_noise)
            ang%=2.0*pi
